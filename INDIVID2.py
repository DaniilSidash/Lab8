    ...
    while True:
        ...
        if command == 'exit':
            ...
            if selected:
                ...
                print("Таких рейсов назначения не найдено.")


        elif command.startswith('load '):
            parts = command.split(maxsplit=1)
            airplanes = load(parts[1])

        elif command.startswith('save '):
            parts = command.split(maxsplit=1)
            save(airplanes, parts[1])

        elif command == 'help':
            print("Список команд:\n")
            print("add - Добавить данные;")
            print("list - Вывести данные;")
            print("select <Рейс> - Вывести всю информацию  по рейсу;")
